...
| manifest `duplicateRecords` | how many records the generator wrote twice on purpose, as a retrying producer would (§4). The totals in the manifest count each record once. Completeness says whether a run tested this; it does not stop a run that did not |
...
| `design` | what the build should contain, checked against the running job after the completeness drain: `operators`, `inputs` and `outputs`, each a list. An operator is found if its text appears, ignoring case, anywhere in the running plan's vertex names (Flink's `[id]` numbers removed). It is a text search, not an exact match, so declare a name specific enough to be found only where you mean: `sink-positions-by-symbol`, not `join`. For a SQL job declare the sink table's short name (`positions_by_symbol`), not `default_catalog.default_database.positions_by_symbol`, which the plan never shows |
| `design.every` | optional, `{topic: interval}`: how often the build writes an output it throttles or windows, e.g. `{"market-values-by-symbol": "10 s"}`. Printed on that output in the job graph. It is what the build declares; the harness does not measure it |
| `flinkProperties` | optional. Extra Flink settings, applied to the job manager and to every task manager. **It is read when the stack comes up, so it has to be in `pipeline.json` before the first `up`** — it reaches the containers through `FLINK_PROPERTIES` in the generated compose file, which is baked in at creation. Adding a setting afterwards changes nothing and says nothing: clean-room run 45 added `pipeline.max-parallelism` after `up`, watched preflight's own key row pass (it reads this file, not the engine), ran a whole completeness drain, and found out only when the design diff read 128 back off the running job. That cost a fill and a drain. This is how a metrics reporter gets in -- section 7 asks for panels that need one, and the rest of the properties are the harness's. Anything that decides what is being measured stops the run: slots, parallelism, pipeline memory, the checkpoint store, and `metrics.reporter.slf4j.*`, which is where busy, idle and back-pressure are read from. Add settings, do not replace these |
