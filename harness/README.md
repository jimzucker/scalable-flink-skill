...
| `flinkProperties` | optional. Extra Flink settings, applied to the job manager and to every task manager. This is how a metrics reporter gets in -- section 7 asks for panels that need one, and the rest of the properties are the harness's. Anything that decides what is being measured is refused: slots, parallelism, worker memory, the checkpoint store, and `metrics.reporter.slf4j.*`, which is where busy, idle and back-pressure are read from. Add settings, do not replace these |
